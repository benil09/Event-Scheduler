...
export async function startWorkflow(workflowName, workflowId, args) {
    if (!TEMPORAL_ENABLED) {
...
export async function sendBookingConfirmationEmailWorkflow(bookingId) {
    return startWorkflow('sendBookingConfirmationEmailWorkflow', `send-booking-confirmation-email-${bookingId}`, [bookingId]);
}
...
export async function createGoogleCalendarEventWorkflow(bookingId) {
    return startWorkflow('createGoogleCalendarEventWorkflow', `create-google-calendar-event-${bookingId}`, [bookingId]);
}
