...
import { findUserByEmail, createUnverifiedHostUser, updateUserOtpAndCredentials, markUserVerified, } from '../repositories/auth.repository.js';
...
export async function signupHostService(data) {
...
export async function loginHostService(data) {
    const email = data.email.toLowerCase().trim();
    const user = await findUserByEmail(email);
    if (!user) {
